...
export default function FlashSaleSectionPremium({ products }: { products: Product[] }) {
  ...
  const saleProducts = products.filter((p) => p.stock > 0).slice(0, 8);
    ...
    <section className="relative py-24 lg:py-32">
      ...
      <div className="container-store relative">
        <motion.div
          ...
          className="relative overflow-hidden rounded-[2rem] border border-slate-200/60 bg-slate-950 shadow-[0_40px_100px_-40px_rgba(15,23,42,0.65)] ring-1 ring-white/10 dark:border-white/10"
        >
          ...
          <div className="relative grid lg:min-h-[520px] lg:grid-cols-[minmax(0,42%)_1fr]">
            {/* Sale banner */}
            <div className="relative flex flex-col justify-center border-b border-white/10 p-8 sm:p-10 lg:border-b-0 lg:border-r lg:p-12 xl:p-14">
              <motion.span
                initial={{ opacity: 0, scale: 0.9 }}
                whileInView={{ opacity: 1, scale: 1 }}
                viewport={{ once: true }}
                className="relative inline-flex w-fit items-center gap-2 rounded-full border border-rose-400/40 bg-rose-500/15 px-4 py-2 text-xs font-bold uppercase tracking-[0.2em] text-rose-200 shadow-[0_0_24px_-4px_rgba(244,63,94,0.5)]"
              >
                <motion.span
              ...
              <h2 className="relative mt-8 font-display text-4xl font-bold leading-[1.08] tracking-tight text-white sm:text-5xl">
                {section.title}
              </h2>
              <p className="relative mt-4 max-w-sm text-base leading-relaxed text-white/55">
              ...
              >
                <AnimatedCountdown endAt={endAt} showProgress />
              </motion.div>
              ...
              >
                <Button
                  asChild
                  size="lg"
                  className="group h-12 rounded-full bg-gradient-to-r from-rose-500 via-fuchsia-500 to-violet-500 px-8 text-white shadow-[0_0_32px_-4px_rgba(244,63,94,0.55)] hover:brightness-110"
                >
                  <Link href="/shop">
                    Shop now
                ...
                <Button
                  ...
                  variant="outline"
                  size="lg"
                  className="h-12 rounded-full border-white/25 bg-white/5 text-white backdrop-blur hover:bg-white/10"
                >
            ...
            {/* Products panel */}
            <div className="relative bg-gradient-to-br from-slate-50 via-white to-slate-100/90 p-6 dark:from-zinc-900 dark:via-zinc-950 dark:to-zinc-900 sm:p-8 lg:p-10 xl:p-12">
              ...
              <FlashSaleCarousel products={saleProducts} onQuickView={setQuickView} />
            </div>
          </div>
        </motion.div>
      </div>
